// ============================================================
// WAWE JOURNAL – script.js (TAMAMEN YENİDEN YAZILDI)
// ============================================================
...
// ── SUPABASE BAĞLANTISI ─────────────────────────────────────
// WW_CONFIG window'da tanımlı olmalı (config.js'den gelir)
if (typeof WW_CONFIG === 'undefined') {
  ...
  window.WW_CONFIG = {
    ...
    NOWPAYMENTS_API_KEY: 'TJQZ4RE-4VV4FGD-JY1G44M-YNW1NWD',
    NOWPAYMENTS_IPN_SECRET: 'Ngvk2yr74o7uznfnljM7QI4u1xS8T9oi'
  };
}
...
async function createNowPaymentInvoice(userId, planType, amount, currency, payCurrency) {
  ...
  try {
    const apiUrl = WW_CONFIG.NOWPAYMENTS_ENV === 'sandbox'
      ? 'https://api-sandbox.nowpayments.io/v1'
      : 'https://api.nowpayments.io/v1';
    
    const { data: { user } } = await sb.auth.getUser();
    const selectedPayCurrency = payCurrency || selectedPayMethod || 'BTC';

    const response = await fetch(apiUrl + '/invoice', {
      method: 'POST',
      headers: {
        'x-api-key': WW_CONFIG.NOWPAYMENTS_API_KEY,
      ...
      body: JSON.stringify({
        ...
        ipn_callback_url: 'https://odasapyhtdopbnlfhwde.supabase.co/functions/v1/nowpayment-webhook',
        success_url: window.location.origin + '/dashboard.html',
        cancel_url: window.location.origin + '/settings.html',
        order_id: userId + '_' + planType + '_' + Date.now(),
        order_description: 'Wawe Journal - ' + planType + ' Plan Subscription',
    ...
    if (!response.ok) {
      showToast('Ödeme başlatılamadı: ' + (data.message || 'API hatası'), 'error');
      return null;
    }
...
async function upgradeToPremium(planType, amount, currency, payMethod) {
  ...
  try {
    ...
    const result = await createNowPaymentInvoice(user.id, planType, amount, currency, method);
